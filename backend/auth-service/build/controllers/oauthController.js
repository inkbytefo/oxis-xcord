...
if (process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_SECRET) {
    ...
    passport.use(new GoogleStrategy(googleOptions, async (_accessToken, _refreshToken, profile, done) => {
        try {
            // Kullanıcı var mı kontrol et
            const userResult = await db.query('SELECT * FROM users WHERE google_id = $1', [profile.id]);
            if (userResult.rows.length === 0) {
                // Yeni kullanıcı oluştur
                const result = await db.query('INSERT INTO users (username, email, google_id) VALUES ($1, $2, $3) RETURNING *', [profile.displayName, profile.emails?.[0]?.value || '', profile.id]);
...
if (process.env.GITHUB_CLIENT_ID && process.env.GITHUB_CLIENT_SECRET) {
    ...
    passport.use(new GitHubStrategy(githubOptions, async (_accessToken, _refreshToken, profile, done) => {
        try {
            // Kullanıcı var mı kontrol et
            const userResult = await db.query('SELECT * FROM users WHERE github_id = $1', [profile.id]);
            if (userResult.rows.length === 0) {
                // Yeni kullanıcı oluştur
                const result = await db.query('INSERT INTO users (username, github_id) VALUES ($1, $2) RETURNING *', [profile.displayName || profile.id, profile.id]);
                const user = result.rows[0];
